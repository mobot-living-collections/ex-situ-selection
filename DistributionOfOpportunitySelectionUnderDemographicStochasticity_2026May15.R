...
## TITLE
##
## Distribution of opportunity for selection under the null model of demographic stochastocity.
##
## INTRODUCTION
##
...
##
## This code implements the simulation described in Appendix S1 of Sucher et al., designed to explore
## properties of the distribution of the variance in relative fitness among maternal lines sourced from
## region "r", a quantity described in equation 3 of the main text and hereafter referred to as "Irt". 
## The simulation explores how, under the null model for opportunity for selection, the mean, variance
## and 95% confidence interval of "Irt" are afected by survival probability, initial number of plants
## from region "r", the number of maternal lines and variation in the initial number of plants per
## maternal line.
##
## DATA FILES REQUIRED
##
...
#maternal lines sourced from a given region "r", "Srt.hat", the mean and variance of "Irt" and
#the 95% confidence interval for "Irt".
...
#create vectors with symbol type and size to plot results
...
for(i in 1:length(unique(OS.par$nr))){
	...
	mean.Srt.hat[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==T & OS.par$nr==unique(OS.par$nr)[i]], col=nr.col[i], type="o", pch=nr.pch.even[i],
	cex=nr.point.cex[i])
}
for(i in 1:length(unique(OS.par$nr))){
	points(OS.par$overall.survival[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==F & OS.par$nr==unique(OS.par$nr)[i]],
	mean.Srt.hat[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==F & OS.par$nr==unique(OS.par$nr)[i]], col=nr.col[i], type="o", pch=nr.pch.uneven[i],
	cex=nr.point.cex[i])
}
...
for(i in 1:length(unique(OS.par$nr))){
	points(OS.par$overall.survival[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==T & OS.par$nr==unique(OS.par$nr)[i]],
	mean.Irt[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==T & OS.par$nr==unique(OS.par$nr)[i]], col=nr.col[i], type="o", pch=nr.pch.even[i],
	cex=nr.point.cex[i])
}
for(i in 1:length(unique(OS.par$nr))){
	points(OS.par$overall.survival[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==F & OS.par$nr==unique(OS.par$nr)[i]],
	mean.Irt[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==F & OS.par$nr==unique(OS.par$nr)[i]], col=nr.col[i], type="o", pch=nr.pch.uneven[i],
	cex=nr.point.cex[i])
}
axis(1, at=seq(0.1, 0.9, 0.1), cex.axis=1.5)
mtext(side=3, bquote(italic(N[r]) == .(Nr.to.plot)), cex=1.5)
...
legend(0.5, 5.5, col=nr.col, lty=1, pch=c(21, 19, 2, 17),
	cex=1.5, pt.cex=rep(c(1.5, 1), times=2),
	c(expression(paste(italic(n[r]), "=10", ", even")),
	expression(paste(italic(n[r]), "=50", ", even")),
	expression(paste(italic(n[r]), "=10", ", uneven")),
...
for(i in 1:length(unique(OS.par$nr))){
	points(OS.par$overall.survival[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==T & OS.par$nr==unique(OS.par$nr)[i]],
	var.Irt[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==T & OS.par$nr==unique(OS.par$nr)[i]], col=nr.col[i], type="o", pch=nr.pch.even[i],
	cex=nr.point.cex[i])
}
for(i in 1:length(unique(OS.par$nr))){
	points(OS.par$overall.survival[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==F & OS.par$nr==unique(OS.par$nr)[i]],
	var.Irt[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==F & OS.par$nr==unique(OS.par$nr)[i]], col=nr.col[i], type="o", pch=nr.pch.uneven[i],
	cex=nr.point.cex[i])
}
axis(1, at=seq(0.1, 0.9, 0.1), cex.axis=1.5)
mtext(side=3, bquote(italic(N[r]) == .(Nr.to.plot)), cex=1.5)
...
plot(OS.par$overall.survival[OS.par$Nr==Nr.to.plot], length.95CI.Irt[OS.par$Nr==Nr.to.plot], xlim=c(0.1,0.9), ylim=c(0,2.2),
	...
	#xlab=expression(paste("Survival probability for plants from region ", italic(r), " (", italic(S[or]), ")", sep="")),
	xlab="",
	ylab=expression(paste("Length of 95% CI for ", italic(I[r](t)), sep="")),
	cex.lab=1.5, cex.axis=1.5)
for(i in 1:length(unique(OS.par$nr))){
	points(OS.par$overall.survival[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==T & OS.par$nr==unique(OS.par$nr)[i]],
	length.95CI.Irt[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==T & OS.par$nr==unique(OS.par$nr)[i]], col=nr.col[i], type="o", pch=nr.pch.even[i],
	cex=nr.point.cex[i])
}
for(i in 1:length(unique(OS.par$nr))){
	points(OS.par$overall.survival[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==F & OS.par$nr==unique(OS.par$nr)[i]],
	length.95CI.Irt[OS.par$Nr==Nr.to.plot & OS.par$even.Nri.0==F & OS.par$nr==unique(OS.par$nr)[i]], col=nr.col[i], type="o", pch=nr.pch.uneven[i],
	cex=nr.point.cex[i])
}
axis(1, at=seq(0.1, 0.9, 0.1), cex.axis=1.5)
mtext(side=3, bquote(italic(N[r]) == .(Nr.to.plot)), cex=1.5)
mtext(expression(paste("Survival probability, ", italic(K[r])/italic(N[r]), sep="")), side=1,
	line=3.5, cex=1.5)
legend(0.5, 1.9, col=nr.col, lty=1, pch=c(21, 19, 2, 17),
	cex=1.5, pt.cex=rep(c(1.5, 1), times=2),
	c(expression(paste(italic(n[r]), "=10", ", even")),
	expression(paste(italic(n[r]), "=50", ", even")),
	expression(paste(italic(n[r]), "=10", ", uneven")),
	expression(paste(italic(n[r]), "=50", ", uneven"))))
#mtext(side=2, "a)", at=2.4, line=3, cex=1.5, las=2)
#mtext(side=2, "b)", at=2.4, line=3, cex=1.5, las=2)
#mtext(side=2, "c)", at=2.4, line=3, cex=1.5, las=2)
