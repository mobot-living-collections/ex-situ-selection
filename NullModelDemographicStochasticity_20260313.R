...
Qa.prov.data$GeographicRegion <- factor(Qa.prov.data$GeographicRegion)

bar.col <- rep(viridis(n = length(unique(Qa.prov.data$GeographicRegion)),
                       option = "D",
                       direction = 1,
                       ),
               times = c(sum(Qa.prov.data$GeographicRegion == 2),
                         sum(Qa.prov.data$GeographicRegion == 3),
                         sum(Qa.prov.data$GeographicRegion == 4),
                         sum(Qa.prov.data$GeographicRegion == 5),
                         sum(Qa.prov.data$GeographicRegion == 8)))
png(filename = "Fig1_a.png",
    width = 10, height = 7, units = "in", res = 600)


  par(mar = c(5, 5, 4, 2) + 0.1)
  barplot(plants.per.accession[region.plants.order], ylim = c(0, 11),
        names.arg = "", cex.lab = 1.5,
        cex.axis = 1.5, col = bar.col,
        las = 1,
        axes = FALSE,
        xlab = expression("Region, " * italic(r)),
        ylab = expression(paste("Initial plants, ", italic(N(0)[italic(ri)]))))
  axis(2, at = 0:10, las = 1, cex.axis = 1.5)

# Edit vertical axis
#axis(2, at = seq(0, 10, 1), labels = TRUE, cex.axis = 1.5)

# Axis and region labels for regions 2, 3, 4, 5, and 8
  ml.r2 <- sum(Qa.prov.data$GeographicRegion == 2)
  axis(1, at = c(0.2, ml.r2 * (1 + 0.2)), labels = FALSE, line = 0.5, lwd = 1.5)
  ...
  axis(1, at = c(0.2 + ml.r2 * (1 + 0.2) + ml.r3 * (1 + 0.2) + ml.r4 * (1 + 0.2),
               ml.r2 * (1 + 0.2) + ml.r3 * (1 + 0.2) + ml.r4 * (1 + 0.2) + ml.r5 * (1 + 0.2)),
     labels = FALSE, line = 0.5, lwd = 1.5)
  ...
  axis(1, at = c(0.2 + ml.r2 * (1 + 0.2) + ml.r3 * (1 + 0.2) + ml.r4 * (1 + 0.2) + ml.r5 * (1 + 0.2),
               ml.r2 * (1 + 0.2) + ml.r3 * (1 + 0.2) + ml.r4 * (1 + 0.2) + ml.r5 * (1 + 0.2) + ml.r8 * (1 + 0.2)),
     labels = FALSE, line = 0.5, lwd = 1.5)
  mtext("8", side = 1, at = ml.r2 * (1 + 0.2) + ml.r3 * (1 + 0.2) + ml.r4 * (1 + 0.2) + ml.r5 * (1 + 0.2) + 0.2 + (ml.r8 * (1 + 0.2)) / 2, line = 1.2, cex = 1.5)

# Add axis 1 title
#mtext("Region, " [italic(r)], side = 1, line = 3, cex = 1.5)

  # Add legend
  legend(16 * (1 + 0.33), 11, paste("Maternal lines region", c(2, 3, 4, 5, 8)),
       fill = viridis(n = length(unique(Qa.prov.data$GeographicRegion)), option = "D"), cex = 1.2)
  mtext(side = 2, "a)", cex = 1.5, las = 1, at = 11.25, line = 2, adj = 0)
...
png(filename = "surviving_plants_per_accession.png",
...
#add legend
#legend(16.5*(1+0.2),11, paste("Maternal lines region", c(2,3,4,5,8)),
       #fill=viridis(n = length(unique(Qa.prov.data$GeographicRegion)), option = "D"), cex=1.2)

mtext(side = 2, "b)", cex = 1.5, las = 1, at = 11.25, line = 2, adj = 0)
...
png(filename = "Fig1_b.png",
    width = 10, height = 7, units = "in", res = 600)



par(mar=c(5, 5, 4, 2) + 0.1)
barplot(surviving.plants.per.accession[region.plants.order], ylim=c(0,11),
        names.arg="", cex.lab=1.5, cex.axis=1.5, col=bar.col,
        las = 1,
        axes = FALSE,
        xlab = expression("Region, " * italic(r)),
        ylab = expression(paste("Surviving plants, ", italic(N(t)[italic(ri)])), sep="")
        )
axis(2, at = 0:10, las = 1, cex.axis = 1.5)
#edit vertical axis
#axis(2, at=seq(, 10, 1), labels=T, cex.axis=1.5)
...
#add legend
legend(5*(2+0.5), 11, "Demographic stochasticity null model", pch=NA, cex=1.2)
#add symbol (a bit fidgety)
arrows(14, 10.2, 14, 10.8, length=0.01, angle=90, code=3, col="black")
mtext(side = 2, "b)", cex = 1.5, las = 1, at = 11.25, line = 2, adj = 0)
...
# combine into grid for a and b

library(png)
# Read in the saved PNG files
img1 <- readPNG("Fig1_a.png")
img2 <- readPNG("Fig1_b.png")

# Open a new PNG device for the combined figure
png("figure1.png", width = 16, height = 6, units = "in", res = 600)

# Set up 1 row, 2 column layout with no margins between panels
