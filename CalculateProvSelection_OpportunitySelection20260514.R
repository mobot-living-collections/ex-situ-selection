...
## DATA FILES REQUIRED
##
## 1. "QaSurDat_2025Sep02_135300.csv" [Survival data]
...
Qa.surv.data <- read.table("QaSurDat_2025Sep02_135300.csv", header = T, sep =",")
dim(Qa.surv.data)
...
region.2.plants <- plants.per.accession[Qa.prov.data$GeographicRegion==2]
region.3.plants <- plants.per.accession[Qa.prov.data$GeographicRegion==3]
region.4.plants <- plants.per.accession[Qa.prov.data$GeographicRegion==4]
region.5.plants <- plants.per.accession[Qa.prov.data$GeographicRegion==5]
region.8.plants <- plants.per.accession[Qa.prov.data$GeographicRegion==8]
...
png(filename = "Fig3_20260320.png", 
...
plot(maternal.lines.per.provenance, mean.null.mean.survival.rate.per.provenance, xlim=c(0, 16), ylim=c(0, 1),
     bty="n", cex.axis=1.5, cex.lab=1.5, xlab=expression("Number of maternal lines, "* italic(n[r])), 
     ylab=expression(paste("Mean survival rate, ", italic(bar(S[r])(t)), sep="")), type="n",
     axes = FALSE)
axis(1, at = c(0, 5, 10, 15), las = 1, cex.axis = 1.5)
axis(2, at = seq(0,1,by=0.1), las = 1, cex.axis = 1.5)
...
points(maternal.lines.per.provenance, mean.null.mean.survival.rate.per.provenance, pch=19, col="gray60", cex=1)
...
#try with a dashed line instead of the circle for the median
segments(maternal.lines.per.provenance - 0.6, median.null.mean.survival.rate.per.provenance,
         maternal.lines.per.provenance + 0.5, median.null.mean.survival.rate.per.provenance,
         lty = 3, col="gray70", lwd=2)
...
#observed values
legend(x=5.5, y=1.1,
       legend = c(expression(paste("Observed region 2, ", italic(p), " = 0.838")),
                  expression(paste("Observed region 3, ", italic(p), " = 0.53")),
                  expression(paste("Observed region 4, ", italic(p), " = 0.838")),
                  expression(paste("Observed region 5, ", italic(p), " = 0.838")),
                  expression(paste("Observed region 8, ", italic(p), " = 0.565"))),
...
arrows(6, 0.16, 6, 0.20, length=0.08, angle=90, code=3, col="gray70")
dev.off()
#############################################################################################
# 4. Calculate opportunity for selection within each provenance (i.e., geographic region).
#############################################################################################
...
png(filename = "OppSelection_a.png",
    width = 10, height = 7, units = "in", res = 600)
...
text(observed.opportunity.for.selection.region2, 670, "Observed region 2", cex=1.5, xpd=TRUE, col="#440154FF")
mtext(side=2, "a)", cex=1.5, las=2,  at=650, line=4, adj=0)
text(0.5, 400, expression(paste(italic(p), " = 0.349")), cex=1.5)
...
#png(filename = "OppSelection_b.png",
...
text(observed.opportunity.for.selection.region3-0.25, 5750, "Observed region 3", cex=1.5, xpd=TRUE, col="#3B528BFF")
mtext(side=2, "b)", cex=1.5, las=2,  at=5500, line=4, adj=0)
text(2.5, 3700, expression(paste(italic(p), " = 0.067")), cex=1.5)
...
#png(filename = "OppSelection_c.png",
 #   width = 10, height = 7, units = "in", res = 300)
...
text(observed.opportunity.for.selection.region4, 700, "Observed region 4", cex=1.5, xpd=TRUE, col="#21908CFF")
mtext(side=2, "c)", cex=1.5, las=2,  at=650
      , line=4, adj=0)
text(0.8, 500, expression(paste(italic(p), " = 0.624")), cex=1.5)
...
#png(filename = "OppSelection_d.png",
 #   width = 10, height = 7, units = "in", res = 300)
...
text(observed.opportunity.for.selection.region5, 630, "Observed region 5", cex=1.5, xpd=TRUE, col="#5DC863FF")
mtext(side=2, "d)", cex=1.5, las=2,  at=645, line=4, adj=0)
text(0.6, 400, expression(paste(italic(p), " = 0.624")), cex=1.5)
...
# combine OppSelection files for the 4 grid layout
library(png)
img1 <- readPNG("OppSelection_a.png")
img2 <- readPNG("OppSelection_b.png")
img3 <- readPNG("OppSelection_c.png")
img4 <- readPNG("OppSelection_d.png")

png("figure4_grid.png", width = 16, height = 12, units = "in", res = 600)
